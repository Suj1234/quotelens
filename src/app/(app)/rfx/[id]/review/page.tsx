...
export default async function ReviewPage({ params, searchParams }: PageProps<"/rfx/[id]/review">) {
  ...
  const canAct = user.role !== "approver";
  const [items, pending] = await Promise.all([listReview(id), canAct ? pendingReplies(id) : Promise.resolve(null)]);
  return (
    <div className="page">
      <ReviewQueue rfxId={id} items={items} pending={pending} canAct={canAct} focus={typeof item === "string" ? item : null} vendor={typeof vendor === "string" ? vendor : ""} />
    </div>
  );
}
