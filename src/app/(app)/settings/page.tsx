...
export default async function SettingsPage({ searchParams }: PageProps<"/settings">) {
  ...
  const [settings, changes, eligible, calls, rfx, vendors] = await Promise.all([
    ...
    db().from("rfx").select("id, code").order("code"), db().from("vendors").select("id, short_code, name, city").order("name"),
  ]);
  ...
  return (
    <div className="page">
      <h1>Settings</h1>
      <p className="text-muted-foreground" style={{ marginTop: 4 }}>Transport, decision layer, rates, the category template the co-pilot works from, and the two pages that check the system on itself.</p>
      <SettingsCards settings={settings} jevKey={!!process.env.OPENROUTER_API_KEY} category={CATEGORY}
