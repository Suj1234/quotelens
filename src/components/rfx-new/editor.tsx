...
// DESIGN §3.3 editor card: Lines [n] · Terms · Questionnaire [n] · Vendors [n]; everything editable, saved by "Save draft".
...
export function Editor({ edit, set, draft, sourceFile, onAttach }: {
  edit: Edit; set: (fn: (e: Edit) => Edit) => void; draft: Draft | null; sourceFile: string | null; onAttach: () => void;
}) {
  ...
  return (
    <div className="card" style={{ display: "flex", flexDirection: "column", minHeight: 520, minWidth: 0 }}>
        ...
        <div className="bd" style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div className="empty" style={{ maxWidth: 420 }}>
            ...
            <div style={{ marginTop: 12, display: "flex", gap: 8, justifyContent: "center" }}>
              <Button size="sm" onClick={onAttach}>Attach last year&apos;s sheet</Button>
              ...
              <tbody>
                {edit.lines.map((l, i) => (
                  <tr key={i}>
                    ...
                    <td><select className="sel" value={l.item_type} onChange={(e) => setLine(i, { item_type: e.target.value })}>{["", "box", "sheet", "partition", "other"].map((t) => <option key={t} value={t}>{t || "—"}</option>)}</select></td>
      ...
      {tab === "terms" && (
        <div className="bd">
          <dl className="kv" style={{ gridTemplateColumns: "170px 1fr", alignItems: "center" }}>
            ...
            <dt>Freight</dt><dd><label style={{ display: "inline-flex", gap: 6, alignItems: "center", fontSize: 12.5 }}><input type="checkbox" checked={h.freight_included_requested} onChange={(e) => setH({ freight_included_requested: e.target.checked, terms_set: true })} /> included in the price</label></dd>
            <dt>Payment terms (days)</dt><dd><Num v={h.payment_terms_days} on={(v) => setH({ payment_terms_days: v ?? 0, terms_set: true })} /></dd>
            <dt>Quote validity (days)</dt><dd><Num v={h.validity_days_requested} on={(v) => setH({ validity_days_requested: v ?? 0, terms_set: true })} /></dd>
            <dt>Contract (months)</dt><dd><Num v={h.contract_months} on={(v) => setH({ contract_months: v ?? 0, terms_set: true })} /></dd>
            <dt>Response deadline</dt><dd><input type="date" className="inp mono" value={h.response_deadline ?? ""} onChange={(e) => setH({ response_deadline: e.target.value || null, terms_set: true })} /></dd>
            <dt>Delivery locations</dt><dd><Txt v={h.delivery_locations.join(", ")} on={(v) => setH({ delivery_locations: v.split(",").map((x) => x.trim()).filter(Boolean), terms_set: true })} /></dd>
          ...
          {!h.terms_set && <p className="hint" style={{ marginTop: 10 }}>Terms are pending: ask the co-pilot for &quot;standard terms&quot; or change any field here.</p>}
        </div>
      )}

      {tab === "questionnaire" && (
        <div className="bd" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {edit.questions.length === 0 && <div className="empty"><b>No questionnaire yet.</b><div style={{ fontSize: 12, marginTop: 6 }}>Ask the co-pilot to propose one; you mark which questions are mandatory and which disqualify.</div></div>}
          {edit.questions.map((q, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "28px 1fr 104px 92px 118px auto", gap: 8, alignItems: "center" }}>
              <span className="mono text-muted-foreground">Q{i + 1}</span>
              <Txt v={q.text} on={(text) => setQ(i, { text })} />
              <select className="sel" value={q.answer_type} onChange={(e) => setQ(i, { answer_type: e.target.value as EditQuestion["answer_type"] })}><option value="yes_no">yes / no</option><option value="number">number</option><option value="text">text</option></select>
              <label style={{ fontSize: 12, display: "inline-flex", gap: 5, alignItems: "center" }}><input type="checkbox" checked={q.mandatory} onChange={(e) => setQ(i, { mandatory: e.target.checked })} /> mandatory</label>
              <input className="inp mono" placeholder={q.answer_type === "number" ? "e.g. lt:200" : "disqualify if…"} title='Disqualify if: "no" / "yes" for yes-no questions, "lt:200" / "gt:30" for numbers; empty = never' value={q.disqualify_if} onChange={(e) => setQ(i, { disqualify_if: e.target.value })} />
              <Button size="xs" variant="ghost" onClick={() => set((e) => ({ ...e, questions: e.questions.filter((_, j) => j !== i) }))}>Remove</Button>
            </div>
          ))}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6 }}>
            <span className="hint">◆ disqualifying = a filled &quot;disqualify if&quot;. Attached to the dispatch as a PDF.</span>
      ...
      {tab === "vendors" && (
        <div className="bd" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {vendorCount === 0 && <div className="empty"><b>No vendors yet.</b><div style={{ fontSize: 12, marginTop: 6 }}>Add them from the address book or by name and email.</div></div>}
          {vendorCount > 0 && (
            <table className="t">
              <thead><tr><th>Vendor</th><th>Email</th><th>City</th><th /></tr></thead>
              <tbody>
                {edit.vendors.map((v) => <tr key={v.vendor_id}><td><b>{v.name}</b></td><td className="mono">{v.email}</td><td>{v.city ?? "—"}</td><td><Button size="xs" variant="ghost" onClick={() => set((e) => ({ ...e, vendors: e.vendors.filter((x) => x.vendor_id !== v.vendor_id) }))}>Remove</Button></td></tr>)}
                {edit.newVendors.map((v, i) => <tr key={`n${i}`}><td><b>{v.name}</b> <span className="chip grey">new</span></td><td className="mono">{v.email}</td><td>—</td><td><Button size="xs" variant="ghost" onClick={() => set((e) => ({ ...e, newVendors: e.newVendors.filter((_, j) => j !== i) }))}>Remove</Button></td></tr>)}
              </tbody>
            </table>
          )}
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <span className="eyebrow" style={{ width: 110 }}>Address book</span>
            <select className="sel" value={pick} onChange={(e) => setPick(e.target.value)}><option value="">Choose a vendor…</option>{book.map((b) => <option key={b.vendor_id} value={b.vendor_id}>{b.name}{b.city ? ` · ${b.city}` : ""}</option>)}</select>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <span className="eyebrow" style={{ width: 110 }}>New vendor</span>
            <input className="inp" placeholder="Name" value={nv.name} onChange={(e) => setNv({ ...nv, name: e.target.value })} />
            <input className="inp" placeholder="Email" type="email" value={nv.email} onChange={(e) => setNv({ ...nv, email: e.target.value })} />
